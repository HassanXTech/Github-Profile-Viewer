...
export function GitHubSearchSection({ onSearch }: GitHubSearchSectionProps) {
  const [username, setUsername] = useState("")

  const smoothScrollToProfile = () => {
    const attemptScroll = (attempt = 1, maxAttempts = 5) => {
      const profileSection = document.getElementById("profile-section")

      if (profileSection) {
        const elementRect = profileSection.getBoundingClientRect()
        ...
        console.log("[v0] Smooth scroll executed successfully")
        return
      }

      if (attempt < maxAttempts) {
        console.log(`[v0] Profile section not found, attempt ${attempt}/${maxAttempts}, retrying...`)
        setTimeout(() => attemptScroll(attempt + 1, maxAttempts), 200 * attempt)
      } else {
        console.log("[v0] Profile section not found after all attempts")
      }
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    console.log("[v0] Form submitted with username:", username.trim())

    if (username.trim()) {
      onSearch(username.trim())
      smoothScrollToProfile()
    }
  }

  const handleButtonClick = () => {
    console.log("[v0] Button clicked with username:", username.trim())
  ...
  const handleKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === "Enter") {
      e.preventDefault()
      console.log("[v0] Enter key pressed with username:", username.trim())
  ...
  return (
    <section
      className="flex flex-col items-center text-center relative mx-auto rounded-2xl overflow-hidden mt-12 mb-6 py-0 px-4
         w-full h-[400px] md:w-[1220px] md:h-[600px] lg:h-[810px] md:px-0"
    >
      {/* ... existing SVG background code ... */}
      ...
      <div className="relative z-10 space-y-4 md:space-y-5 lg:space-y-6 mb-6 md:mb-7 lg:mb-9 max-w-md md:max-w-[500px] lg:max-w-[588px] mt-16 md:mt-[120px] lg:mt-[160px] px-4">
        <div className="flex items-center justify-center gap-3 mb-4">
          <Github className="w-8 h-8 md:w-10 md:h-10 text-primary" />
          <h1 className="text-foreground text-3xl md:text-4xl lg:text-6xl font-semibold leading-tight">
            GitHub Profile Viewer
          </h1>
        </div>
        <p className="text-muted-foreground text-base md:text-base lg:text-lg font-medium leading-relaxed max-w-lg mx-auto">
          A simpler way to show your GitHub profile and repositories. Enter any GitHub username to explore their coding
          journey.
        </p>
      </div>

      <form onSubmit={handleSubmit} className="relative z-10 flex flex-col sm:flex-row gap-3 max-w-md mx-auto px-4">
        <Input
          type="text"
          ...
          className="flex-1 bg-card/50 border-border text-foreground placeholder:text-muted-foreground-light"
        />
        <Button
          type="button"
          onClick={handleButtonClick}
          className="bg-primary text-primary-foreground hover:bg-primary/90 px-8 py-3 rounded-md font-medium text-base shadow-lg"
        >
          View Profile
        </Button>
      </form>
